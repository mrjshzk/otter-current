...
func _ready() -> void:
	...
	dialogue_label.started_typing.connect(_on_voice_typing_started)
	dialogue_label.finished_typing.connect(voice.stop)
	dialogue_label.skipped_typing.connect(voice.stop)
...
func _on_voice_typing_started() -> void:
	var settings := _speaker_voice_settings()
	voice.start(
		settings.get("pitch", 1.0),
		settings.get("speed", 1.0),
		settings.get("tone", 0.5),
	)

func _speaker_voice_settings() -> Dictionary:
	for state in temporary_game_states:
		if state is Dictionary:
			continue
		if state != null and state.has_method("get_voice_settings"):
			return state.get_voice_settings()
	return {}
